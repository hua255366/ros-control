...
if __name__ == '__main__':
    ...
    move.forward_trapezoidal(1.34,1.2,2.5)
...
#     time.sleep(1)
    ...
    time.sleep(1)
    move.forward_trapezoidal(0.30,0.2,1)
   ...
   # 机械臂初始化     
    ...
    move.right_trapezoidal(196.5, 0.8, 1.5)  #旋转180度,车头对墙，方便后面测距
   #调整前后距离
    # move.moveforward(0.05,1.5)
    for _ in range(5):  #8
        distance = Sonor().get_sonor_data()
    ...
    move.backward_trapezoidal(0.57, 0.6, 3)

    # 根据物体类型放置
    x, y, width, height, label = detect_result
    if label == "clock":##
        basic.move_to_my_coords(grabParams.coords_place_clock,80,2)
        basic.grap(False)
        time.sleep(0.5)
        mc.send_angle(Angle.J2.value, 20, 30)
    else:
        basic.move_to_my_coords(grabParams.coords_place_apple,80,1)
        basic.grap(False)
        time.sleep(0.5)
        mc.send_angle(Angle.J2.value, 20, 30)
    ...
    move.forward_trapezoidal(0.55,0.2,1)   
...
# # # 第二次
    ...
    #抓取物体
    goal1_gab(detect_result)
    move.backward_trapezoidal(0.12,0.1,0.5)
 ...
 # # # 第三次
    ...
    move.forward_trapezoidal(0.12,0.08,1)
   ...
   #调整前后距离
    ...
    move.left_trapezoidal(98.23, 0.8, 1.5)
    time.sleep(0.5)
    move.backward_trapezoidal(0.54, 0.6, 3)
    ...
    # 返回准备位置 
    basic.move_to_my_angles(grabParams.angles_ready,60,1.5)
    move.forward_trapezoidal(0.51,0.2,1)   
    basic.move_to_my_coords(grabParams.coords_ready,80,0)
...
# # # 第四次
    ...
    move.left_trapezoidal(98.23, 0.8, 1.5)
    time.sleep(1)
    move.forward_trapezoidal(0.18,0.08,0.1)
    ...
    move.backward_trapezoidal(0.30,0.1,0.5)
    time.sleep(0.5)
    move.right_trapezoidal(196.5, 0.8, 1.5)  #旋转两个90度，即180度,车头对墙，方便后面测距

   #调整前后距离
    # move.forward_trapezoidal(0.2,0.4,0.5)
    # move.moveforward(0.05,1.5)
    for _ in range(5):  #8
        distance = Sonor().get_sonor_data()
        print(distance)
    ...
    move.left_trapezoidal(100,1.2,1)
