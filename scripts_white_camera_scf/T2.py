...
CLASSES = ("apple", "clock","banana","cat ","bird ",) #[0,1,2,3,4]#banana_ban
...
############################################
	...
	time.sleep(2)	
       ...
       #######################################
        if coords_target[0] >= 230:
            coords_target[0] = coords_target[0]-40
  
        

        # 返回准备阶段##############all has profited by charlie
        #time.sleep(0.5)#1####
        #coords_target1 = [coords_target[0], coords_target[1]-5, height_bias+120, coords_target[3], coords_target[4], coords_target[5]]#coords_target[1]-30############high50
        #print("coords_target1=",coords_target1)
        #basic.move_to_target_coords(coords_target1, grabParams.GRAB_MOVE_SPEED)
        # 放置动作
        #time.sleep(0.5)#1####

        if sum == 0  and put_cat == 0:
            #目的地
            #basic.move_to_target_coords([n_final+int(x), m_final+int(y), height_bias, -155, coords[4], coords[5]], grabParams.GRAB_MOVE_SPEED)
	...
	print("mafumafumafumafumafumafumafumafumafumafumafumafumafu")
    ...
    def obj_detect(self, img):
        ...
        if x+y > 0 and (classes[0]==0 or classes[0]==4):
            return x, y,classes
        else:
            angles = [0, 0, 0, 0, 0, 0]
            self.mc.send_angles(angles,60)
            return None
