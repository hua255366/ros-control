...
###########################################################################  放置坐标 angles_put
angles_put = [6.85,35.68,-143.52,28.65,-1.4,145.19]#[123,-56.9,140.1,-177.53,-0.24,-138.3] speed=15[0.61,-0.17,4.3,-0.61,-0.7,0.52]
###########################################################################  放置过渡
angles_guo = [0,0,0,0,0,0]#[3.77,19.51,-142.99,32.16,-2.54,51.67]
...
# zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx
    ...
    def map_choice_of_positions(self, i):
        ...
        positions_red_left = [(-0.08,0.2,-1.57), # 0: middle_Obstacle_Left_Side #
                             (1.555, 0.638, 1.682),  # 1: shelf_front#1.805, 0.390,1.787#1.644, 0.363,1.733#1.543, 0.442, 1.663 shelf_middle
...
class MoveForwardUntilDetected:
    ...
    def execute_command_and_restart(self, result_class):
        ...
        ######################################################################################### 放下(小车前端）
        self.mc.send_angles(angles_guo,Speed)
        time.sleep(3)
        basic.grap(False)
        time.sleep(1)
        ...
        frame = cap.read()
        self.detect.obj_detect(frame)#################################### 打开摄像头
        navigator.run2(navigator.map_choice_of_positions(5))
        self.execute_command_and_restart1()
        ...
        m_speed.moveforward()#################################################################  抓完第二方块前进至shelf_middle
        ...
        # 后退
        m_speed.moveback(0.8, 0.25)        # 后退(3.2,0.18)############################ 后退至desk
        print("后退")
        time.sleep(0.5)
...
#################################################################################################################
        ...
        if self.execution_count >= 4 or time.time() - self.start_time > 420: # 480秒等待时间######################### home的召令
            ...
            # 后退
            m_speed.moveback(0.9,0.4)############################# 自己丈量
            print("后退")
            # 旋转
            m_speed.rotate_to_left(4, 0.75)
            time.sleep(0.3)

            m_speed.moveback()####################################################### 小后退
            ...
            m_speed.moveback()######################################################### 退到shelf_middle
    ...
    def execute_command_and_restart1(self):
        """执行抓取放置后后重新开始前进检测"""
        # 准备抓取
        self.mc.send_angles(angles_ready_grab, Speed)
        print("执行抓取准备动作")
    ...
    def moveforward_runtime(self):###############################################################
        ...
        navigator.run2(navigator.map_choice_of_positions(1)) # 1: shelf_front（middle)
