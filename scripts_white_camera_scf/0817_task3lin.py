...
###########################################################################  放置坐标
angles_middle=[166.7, -61.1, 274.1, -179.84, 2.56, -138.22]
angles_put=[123,-56.9,140.1,-177.53,-3.97,125.73]
...
#############################################catch the thing
    ...
    def execute_comand_second(self):
        self.mc.send_angles(angles_ready_grab, Speed)
        print("执行抓取准备动作")
        time.sleep(1)
        ...
        while not rospy.is_shutdown() and not self.target_detected:
            ...
            if (target_info is not None) and (self.forward_time < (15-self.execution_count*2)):
                ...
                if y is not None and (screen_midpoint-45) < y < (screen_midpoint+35):
                    ...
                    self.stop_movement()
                    #self.execute_command_and_restart(result_class)  # 执行命令并准备重新开始#####trying
                    self.execute_comand_first()
                    break
