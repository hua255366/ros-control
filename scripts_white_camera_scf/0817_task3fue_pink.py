...
[3.95, 24.87, -143.7, 33.04, -2.37, 145.28]
...
obstacle_side = 7 # if obstacle on left side, obstacle_side = 0; elif on right side, obstacle_side = 7
...
angles_put_sec=[3.95, 24.87, -143.7, 33.04, -2.37, 145.28]#[10.54, 32.78, -140.88, 16.25, -5.09, 150.38]
...
# zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx
    ...
    def map_choice_of_positions(self, i):
        ...
        positions_blue_right = [(1.234, -0.273, -0.241), # 0: middle_Obstacle_Left_Side 
                                (1.990, -0.772, -1.653),  # 1: shelf_front
                                (-0.33,0.1,3.14), # 2: desk
                                (2.201, -0.510,-1.753), # 3: home_prepare (middle向home转)
                                (-0.176, 0.095, 3.039), # 4: home
                                (0.17,0.1+0.06*(move_forward.get_execution_count()-1),0), # 5: restart_1 (shelf_front的y平移)
                                (-0.07, 0.05, 3.14), # 6: timeout_1 ((middle转半圈))
                                (1.667, -0.715,-0.108)]# 7: middle_Obstacle_Right_Side
        ...
        positions_red_left = [(-0.08,0.2,-1.57), # 0: middle_Obstacle_Left_Side #
                             (1.555, 0.638, 1.682),  # 1: shelf_front#1.805, 0.390,1.787#1.644, 0.363,1.733#1.543, 0.442, 1.663
                             (1.859, 0.251,-1.450), # 2: desk#1.865, 0.171,-1.440
                            (0.329, 0.087,-3.043), # 3: home_prepare (middle向home转)#-0.400, -0.054,1.837
                            (-0.429, -0.243, -2.932), # 4: home#-0.595, 1.000, 1.821
...
class MoveForwardUntilDetected:
	...
	mafumafu+=1
        ...
        if put_class == 0:
            ...
            time.sleep(1)
            self.mc.send_angles(angles_put_sec,Speed+15)
            time.sleep(2)
            basic.grap(True)
            time.sleep(2)
            self.mc.send_angles(angles_middle,Speed+15)
...
# 抬起
	...
	if change_mode==1:
			if mafumafu==1:
        			m_speed.moveback(1, 0.20)        # 后退(3.2,0.18)				
        ...
        # 后退
        		m_speed.moveback(4, 0.12)        # 后退(3.2,0.18)
        		print("后退")
        		time.sleep(0.5)
			...
			m_speed.moveforward(4, 0.12)        # forward(3.2,0.18)
       		 	print("后退")
       			time.sleep(0.5)
...
################################################################fang
	if change_mode==2:
        	mc.send_angles(angles_ready_grab, Speed)
        	time.sleep(1)
        	print("抬起机械臂准备")
