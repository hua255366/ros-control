...
# 准备坐标
angles_ready_grab = [(map_choice * 90.0), -36.56, 70.53, -15.45, 0, -45]#-30.45

# 抓取坐标
angles_grab = [(map_choice * 90.0), -60.56, 44.12, 30, 0, -45]#-65.56   44.12

###########################################################################  放置坐标
angles_middle=[0,0,0,0,0,0]#[3.77, 19.51, -142.99, 32.16, -2.54, 51.67]#[0.0, -0.52, -1.23, -1.05, -0.7, -0.61]
angles_put=[5.88,28.38,-150.99,34.45,-2.63,143.26]#[3.77, 19.51, -142.99, 31.26, -2.54, 51.67]#[7.91, 44.64, -138.07, 10.81, -2.72, 54.4]
angles_put_thr=[27.33, 18.01, -129.19, 7.82, 7.73, -13.18]############################
angles_put_for=[62.31, 35.59, -146.51, 24.34, -13.27, 21.53]#[46.05, 7.2, -136.58, 39.63, -2.37, 2.9]#[3.95, 24.34, -148.18, 31.55, -2.02, 139.74]################################
angles_putdown_sec=[3.95, 24.87, -143.7, 33.04, -2.37, 145.28]#[10.54, 32.78, -140.88, 16.25, -5.09, 150.38]
angles_putdown_thr=[27.33, 18.01, -129.19, 7.82, 7.73, -13.18]###########################
angles_putdown_for=[62.31, 35.59, -146.51, 24.34, -13.27, 21.53]#############################
angles_putdown_transmit=[31.46, 26.1, -101.07, -14.23, 10.63, 171.29]#########################
mafumafu=0
...
# 放下坐标
angles_putdown = [-11.16,82.7,5.09,86.92,-1.84,-42.09]
...
# zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx
    ...
    def map_choice_of_positions(self, i):
        ...
        # red_left_make
        # positions_red_left  =  [(0.656, -0.317,-1.57), # 0: middle 
        ...
        # red_left_scan_0816
        positions_red_left = [(0.912, -0.353,-0.224), # 0: middle_Obstacle_Left_Side #
                             (1.467, 0.210, 1.335),  # 1: shelf_front#1.805, 0.390,1.787#1.644, 0.363,1.733#1.543, 0.442, 1.663
                             (1.859, 0.251,-1.450), # 2: desk#1.865, 0.171,-1.440
                            (1.720, -0.153,-3.134), # 3: home_prepare (middle向home转)#-0.400, -0.054,1.837
                            (-0.788, -0.042, 2.842), # 4: home#-0.595, 1.000, 1.821
                            (1.805,0.390-0.06*(move_forward.get_execution_count()-1),1.787), # 5: restart_1 (shelf_front的y平移)
                            (0, 0, 3.14), # 6: timeout_1 ((middle转半圈))
                            (0.837, 0.167,0.027)] # 7: middle_Obstacle_Right_Side#0.2,0.1,0#-0.125, 0.272,-1.543
...
class MoveForwardUntilDetected:
    ...
    def execute_put_thing(self):

        global hand_class
        global put_class
	global put_class_thr
	global put_class_for
	global mafumafu
	...
	if hand_class == 0:
            angles_putdown[0]+=15
            self.mc.send_angles(angles_putdown,Speed+15)
            time.sleep(1)#3
            basic.grap(False)
            time.sleep(0.5)#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            angles_putdown[0]-=15
        ...
        if put_class == 0:
            angles_putdown[0]+=15
            self.mc.send_angles(angles_middle,Speed+15)
            time.sleep(1)#3
            self.mc.send_angles(angles_putdown_sec,Speed)
            time.sleep(2)#3
            basic.grap(True)
            time.sleep(2)
            self.mc.send_angles(angles_middle,Speed+15)
            ...
            basic.grap(False)
            time.sleep(0.5)
	self.mc.send_angles(angles_top_ready, Speed+15)
	time.sleep(1)
        # print("抬起机械臂准备")
        ...
        if put_class_thr == 0:
            ...
            self.mc.send_angles(angles_middle,Speed+15)
            time.sleep(1)#3
            self.mc.send_angles(angles_putdown_thr,Speed)
            time.sleep(4)#2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            basic.grap(True)
            time.sleep(2)
            self.mc.send_angles(angles_middle,Speed+15)
	...
	time.sleep(1)
        ...
        if put_class_for == 0:
            ...
            self.mc.send_angles(angles_middle,Speed+15)
            time.sleep(1)#3
            #self.mc.send_angles(angles_putdown_transmit,Speed+15)############
            #time.sleep(2)############
            self.mc.send_angles(angles_putdown_for,Speed)
            time.sleep(4)#2
            basic.grap(True)
            time.sleep(2)
            self.mc.send_angles(angles_middle,Speed+15)
	...
	if mafumafu==1:
	    ...
	    print('mafumafu is the most quit boy')
	    os._exit(1)
    ...
    def execute_comand_first(self,input_class):
        ...
        # 抓取时姿势
        self.mc.send_angles(angles_grab, Speed+10)
        time.sleep(2)
        # 抓取
        basic.grap(True)
        print("抓取动作")
	...
	if change_mode==0:
        	put_class=input_class
        	change_mode+=1
        	self.mc.send_angles(angles_middle, Speed)
        	print("middle")
        	time.sleep(1)
        	self.mc.send_angles(angles_put, Speed)
        	time.sleep(3)#4^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        	basic.grap(False)
        	print("放下")
...
#  grab_all
	if change_mode==1:
		put_class_thr=input_class
        	change_mode+=1
        	self.mc.send_angles(angles_middle, Speed)
        	print("middle")
        	time.sleep(1)
        	self.mc.send_angles(angles_put_thr, Speed)
        	time.sleep(4)#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        	basic.grap(False)
        	print("放下")
	...
	if change_mode==2:
		put_class_for=input_class
        	change_mode+=1
        	self.mc.send_angles(angles_middle, Speed)
        	print("middle")
        	time.sleep(1)
        	self.mc.send_angles(angles_put_for, Speed)
        	time.sleep(4)
        	basic.grap(False)
        	print("放下")
...
# 转（向left）
	if change_mode==3:
			if mafumafu==0:
        			m_speed.moveback(0.9,0.22)        #(1, 0.20)#after the situation of grabing two stuffs!!!!!!!!!!  # 后退(3.2,0.18)				
			hand_class=input_class
        		m_speed.rotate_to_left(3.33, 0.5)   # 旋转90度
        		print("旋转（向left）############################################")
        		time.sleep(1)

        # 后退
        		m_speed.moveback(4, 0.13)        # 后退(3.2,0.18)
        		print("后退")
        		time.sleep(0.5)
        		self.execute_put_thing()
			self.mc.send_angles(angles_top_ready, Speed)
			time.sleep(1)
	#  go back to home            		
			navigator.run2(navigator.map_choice_of_positions(3)) # 3: home_prepare
            		navigator.run2(navigator.map_choice_of_positions(4)) # 4: home)
...
#####################################################################
    ...
    def start_movement(self):
        ...
        while not rospy.is_shutdown() and not self.target_detected:
		    ...
		    self.execute_comand_first(result_class)
                    break
