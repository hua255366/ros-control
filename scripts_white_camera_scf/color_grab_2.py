...
import math
from GrabParams2 import grabParams2
import basic
import argparse
...
y_bias = grabParams2.y_bias 
x_bias = grabParams2.x_bias 
height_bias = grabParams2.height_bias

IMG_SIZE = grabParams2.IMG_SIZE

cap_num = grabParams2.cap_num

# show image and waitkey
debug = grabParams2.debug 

coords = grabParams2.coords_ready
done = grabParams2.done

class Detect_marker(object):
    def __init__(self):
        super(Detect_marker, self).__init__()

        self.mc = MyCobot(grabParams2.usb_dev, grabParams2.baudrate)
        self.mc.power_on()
    ...
    def move(self, x, y):
        ...
        basic.move_to_target_coords(coords_target, grabParams2.GRAB_MOVE_SPEED)
    ...
    def init_mycobot(self): 
        ...
        basic.grap(False)      
        time.sleep(1)         
        basic.move_to_target_coords(coords,grabParams2.GRAB_MOVE_SPEED)     
   

    def get_position(self, x, y):
        wx = wy = 0
        if grabParams2.grab_direct == "front":
            wx = (self.c_y - y) * self.ratio
            wy = (self.c_x - x) * self.ratio
        elif grabParams2.grab_direct == "right":
