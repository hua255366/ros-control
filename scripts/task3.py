...
#angles_ready_grab = [-92.63, 19.42, -65.47, 53.24, -0.7, -45]
# 抓取坐标
angles_grab = [(map_choice * 90.0), -65.56, 44.12, 30, 0, -45]
#angles_grab = [-53.8, -180.6,354.8 , -94.13, -45.86, 179.49]
...
# zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx
    ...
    def map_choice_of_positions(self, i):
        ...
        positions_blue_right = [(-0.05,-0.35,1.57), # 0: middle_Obstacle_Left_Side 
                                ...
                                (2.040,-0.204,-1.450),#shelf
                                (1.935,0.340,1.883), # 2: desk
                            
                                (1.742,0.241,-2.857), # 3: home_prepare (middle向home转)
                                (-0.248, -0.078,-2.912), # 4: home
                
                                (2.040,-0.204,-1.450),#shelf
                                (-0.07, 0.05, 3.14), # 6: timeout_1 ((middle转半圈))
                                (2.031,0.275,0.317)]# 7: middle_Obstacle_Right_Side  要改！！！！！！！！！
...
class MoveForwardUntilDetected:
    ...
    def execute_command_and_restart(self, result_class):
        ...
        # 抓取过渡动作
        # self.mc.send_angles([(map_choice * 90.0), -65.56, 90, -29, 0, -45],30)
        # time.sleep(0.5)
        # self.mc.send_angles([(map_choice * 90.0), -65.56, 78, -26, 0, -45],30)
        # time.sleep(0.5)
        # self.mc.send_angles([(map_choice * 90.0), -65.56, 67, -16, 0, -45],30)
        # time.sleep(0.5)
        # self.mc.send_angles([(map_choice * 90.0), -65.56, 57, -6, 0, -45],30)
        # time.sleep(0.5)
        # self.mc.send_angles([(map_choice * 90.0), -65.56, 50, 6, 0, -45],30)
        # time.sleep(0.5)
        # self.mc.send_angles([(map_choice * 90.0), -65.56, 41, 17, 0, -45],30)
        # time.sleep(0.5)
        ...
        # 旋转（向右）
        m_speed.rotate_to_left(3.14, 0.5)   # 向右旋转90度 ！！！！！！！！！！！！ 99999
        print("旋转（向右）")
        time.sleep(1)
        ...
        # 旋转一百八十度
        m_speed.rotate_to_right(3.14, 1)
        time.sleep(0.3)

        # task3 desk (# zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx )
        navigator.run2(navigator.map_choice_of_positions(2)) # 
        ...
        if self.execution_count >= 4 or time.time() - self.start_time > 480: # 480秒等待时间
            ...
            print("后退")
            # 旋转
            m_speed.rotate_to_left(4.18, 0.75)#旋转一百八十度！！！！！！！！
            time.sleep(0.3)
            # 前进
            navigator.run2(navigator.map_choice_of_positions(5)) # 
            ...
            m_speed.rotate_to_right(3.60, 0.5)  # left turn to right
            print("旋转（向左）")
            # time.sleep(1)
    ...
    def moveforward_runtime(self):
        ...
        m_speed.rotate_to_right(24, 0.17)
    ...
    def start_movement(self):
        self.move_cmd.linear.x = 0.025  # 每次开始前重置线速度
        ...
        while not rospy.is_shutdown() and not self.target_detected:
            ...
            if (target_info is not None) and (self.forward_time < (15-self.execution_count*2)):
                ...
                if y is not None and (screen_midpoint-45) < y < (screen_midpoint+35):#可能需要互换！！！！！！！！
...
if __name__ == "__main__":
    try:
        ...
        navigator.run2(navigator.map_choice_of_positions(obstacle_side)) # 0 or 7: middle_Obstacle_Left or Right_Side
        print('go to task3') 
        
        m_speed.rotate_to_left(3.14, 0.5)#右转90度！！！！！！！！！！！！！！
        time.sleep(3)
        ...
        navigator.run2(navigator.map_choice_of_positions(1)) # 1: shelf_front
        print('ready to task3')
        m_speed.rotate_to_right(3.20, 0.5)  # 旋转90度 要改！！！！！！！！往左拐九十度
        #navigator.run2(0,0.05,-1.57)
        time.sleep(0.1)
        m_speed.moveback(0.07, 0.2)
