...
# 准备坐标
angles_ready_grab = [(map_choice * 90.0), -44.29, 66.88, -12.56, 3.69, -48.51]

angles_grab_ready = [-90.52, 4.48, -60.82, 59.67, 10.01, -42.53]
# 抓取坐标
angles_grab = [(map_choice * 90),  -30.67, -6.24, 32.16, -6.67, -42.18]
# angles_grab = [-90.69, -9.84, -46.4, 61.69, 12.21, -43.24]


# 抬起坐标
# angles_raise = [0, -30, 30, 69.43, -13.62, 0]
angles_raise = [0, 0, 0, 0, 0, 0]
# 放下坐标
angles_putdown = [0, 53.34, 54.05, 78.22, 4.83, -45.08]
angles_putcar1 =  [23.64, 42.71, -156.26, 27.86, 14.76, -12.74]#zhongjian
#angles_putcar2 =  [43.68, 17.49, -156.09, 54.93, 9.93, -38.23]#zuobian
angles_putcar2 = [42.18, 8.26, -146.68, 52.38, 15.29, -44.03]
angles_putcar3 =  [2.37, 17.75, -146.25, 42.62, 9.93, 3.86]#youbian
angles_putdownz1 = [4.48, 59.85, 39.19, 103.44, 6.06, -44.38]
#angles_putdowna1 = [-45.43, 84.72, 13.35, 76.99, 11.6, -37.88]
angles_putdowna1 = [-43.45, 88.06, -0.08, 80.15, -7.29, -37.52]
...
# zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx zqx
    ...
    def map_choice_of_positions(self, i):
        ...
        positions_blue_right = [(-0.573, 0.351,2.882)]
...
class MoveForwardUntilDetected: 
    ...
    def execute_command_and_restart1(self , result_class):
        """执行抓取放置后后重新开始前进检测"""
        # 准备抓取
        self.mc.send_angles(angles_ready_grab, Speed)
        print("执行抓取准备动作")
        time.sleep(1)
        # self.mc.send_angles(angles_grab_ready,Speed)
        # time.sleep(1)
        # 抓取时姿势
        ...
        if self.execution_count == 0:
            self.mc.send_angles(angles_putcar1,Speed)
            time.sleep(1.8)
        if self.execution_count == 1:
            self.mc.send_angles(angles_putcar2,Speed)
            time.sleep(1.8)
        if self.execution_count == 2:
            self.mc.send_angles(angles_putcar3,Speed)
            time.sleep(1.8)
    ...
    def execute_command_and_restart2(self, result_class,p1,p2,p3,p4):
        """执行抓取放置后后重新开始前进检测"""
        # 准备抓取
        self.mc.send_angles(angles_ready_grab, Speed)
        print("执行抓取准备动作")
        ...
        time.sleep(1)
        m_speed.moveback(0.52,0.26)
        
        

        time.sleep(1)

        m_speed.rotate_to_left(3.33,0.460)
        time.sleep(1)
        m_speed.moveback(2.6,0.2)
        time.sleep(2)
...
# 放置目标 p4（调整机械臂位置）
        ...
        if p4 == 0:#苹果
            self.mc.send_angles(angles_putdowna1, Speed1)
        if p4 == 1:#钟
            self.mc.send_angles(angles_putdownz1, Speed1)
...
# 松爪（释放目标）
        ...
        time.sleep(0.1)
# 抬起机械臂复位
        self.mc.send_angles(angles_raise, Speed1)
        print("抬起（复位）")
...
# 抬起机械臂复位
        ...
        if self.execution_count >= 4 or time.time() - self.start_time > 420: # 480秒等待时间
            ...
            m_speed.moveforward(0.1,0.3)
            time.sleep(1)
            m_speed.rotate_to_left(3.33,0.51)
            time.sleep(0.8)
            m_speed.moveforward(5.5,0.4)
            navigator.run2(navigator.map_choice_of_positions(0))
            print('go home')
            ...
            time.sleep(1)
            # 旋转
            m_speed.rotate_to_right(4, 0.392)
            time.sleep(0.3)
            print("前进")
    ...
    def start_movement(self):
        ...
        global p3
        global p4
        self.move_cmd.linear.x = 0.023  # 每次开始前重置线速度
        ...
        while not rospy.is_shutdown() and not self.target_detected:
            ...
            if (target_info is not None) and (self.forward_time < (15-self.execution_count*2)):
                ...
                if y is not None and (screen_midpoint-45) < y < (screen_midpoint+35):
                    ...
                    print("p1=",p1)
                    if self.execution_count  != 3:
                        print("cishu",self.execution_count)
                        self.execute_command_and_restart1(result_class)
                        print("restart1")
                    if self.execution_count  == 3: 
                        self.execute_command_and_restart2(result_class,p1,p2,p3,p4)
    ...
    def sonor_start_movement(self):
        self.move_cmd.linear.x = 0.07  # 每次开始前重置线速度
...
if __name__ == "__main__":
    try:
        ...
        m_speed.moveforward(3.7,0.4)
        print('ready to task3')
        time.sleep(1)
        m_speed.rotate_to_left(3.33, 0.508)
        move_forward.sonor_start_movement()
        m_speed.rotate_to_right(3.33, 0.46)
        time.sleep(0.3)
