...
if __name__ == "__main__":
    ...
    while cv2.waitKey(1) < 0 and not done:
        ...
        if detect_result is None:
            ...
            print(real_x, real_y)
            angle_left3 = [43.59, -61.25, -5.62, -20.91, 2.37, 50.0]
            angle_left4 = [35.5, -67.85, 0.35, -19.59, -1.49, 84.9]
            angle_right3 = [-28.38, -72.24, -0.26, -9.05, -1.84, 21.79]
            angle_right4 = [-15.11, -64.42, -0.61, -22.5, -5.97, 36.73]
            #zhx
            # apple0,clock1,banana2,cat3, bird4
            if detect.result_class == 0:
                if placed3:
                    print("我识别到苹果了，我把它放左边。")
                    coords_now = basic.get_coords()
                    if len(coords_now) == 6:
                        coords = coords_now
                        detect.move(real_x + grabParams.x_bias, real_y + grabParams.y_bias)
                    time.sleep(3)
                    detect.put_down(angle_left3, 20)#把物块3放在左边
                    placed3 = False
                else:
                    print("又识别到物苹果了，我把它放右边。")
                    coords_now = basic.get_coords()
                    if len(coords_now) == 6:
                        coords = coords_now
                        detect.move(real_x + grabParams.x_bias, real_y + grabParams.y_bias)
                    time.sleep(3)
                    detect.put_down(angle_right3, 20)  # 把物块3放在右边
            # zhx
            # apple0,clock1,banana2,cat3, bird4
            elif detect.result_class == 4:
                if placed4:
                    print("我识别到小鸟了，我把它放左边。")
                    coords_now = basic.get_coords()
                    if len(coords_now) == 6:
                        coords = coords_now
                        detect.move(real_x + grabParams.x_bias, real_y + grabParams.y_bias)
                        time.sleep(3)
                    detect.put_down(angle_left4, 20)  # 把物块4放在左边
                    placed4 = False
                else:
                    print("又识别到小鸟了，我把它放右边。")
                    coords_now = basic.get_coords()
                    if len(coords_now) == 6:
                        coords = coords_now
                        detect.move(real_x + grabParams.x_bias, real_y + grabParams.y_bias)
                    time.sleep(3)
                    detect.put_down(angle_right4, 20)  # 把物块4放在右边
            else:
                continue
