...
class Detect_marker(object):
    ...
    def init_mycobot(self):
        mc.send_angles([3.6, -2.28, -85.6, 1.49, 8.43, 49.57], 50)
        time.sleep(0.3)
        basic.grap(False)
        time.sleep(0.5)
    ...
    def put_down(self, target, speed):
        ...
        self.mc.send_angles(target, 50)
        time.sleep(0.7)
        basic.grap(False)
        mc.send_angles([0.7, 2.63, -63.8, 9.4, 10.63, 45.79], 50)
        time.sleep(0.1)
    ...
    while cv2.waitKey(1) < 0 and not done:
        ...
        if detect_result is None:
           continue
            ...
            #angle_left1 = [46.05, -70.66, -0.26, -13.18, 7.2, 48.95]
            angle_left1 = [51.15, -34.18, -69.16, 21.09, 8.34, 41.83]
            angle_left2 = [35.5, -67.85, 0.35, -19.59, -1.49, 84.9]
            #angle_right1 = [-28.38, -72.24, -0.26, -9.05, -1.84, 21.79]
            angle_right1 = [-29.26, -62.75, 7.38, -30.93, 2.98, 49.13]
            angle_right2 = [-15.11, -64.42, -0.61, -22.5, -5.97, 36.73]
            if detect.result_class == 0:
                coords_now = basic.get_coords()
                if len(coords_now) == 6:
                    coords = coords_now
                    detect.move(real_x + grabParams.x_bias, real_y + grabParams.y_bias)
                time.sleep(1)
                detect.put_down(angle_left1, 30)
            os._exit()    
