...
angles_putcar =  [2.81, 18.45, -142.73, 36.65, 10.01, -38.23]
...
class MoveForwardUntilDetected: 
    ...
    def execute_command_and_restart1(self , result_class):
        ...
        self.mc.send_angles(angles_putcar,Speed)
        time.sleep(1.8)
    ...
    def execute_command_and_restart2(self, result_class,p1,p2):
        """执行抓取放置后后重新开始前进检测"""
        # 准备抓取
        self.mc.send_angles(angles_ready_grab, Speed)
        print("执行抓取准备动作")
...
# 抬起机械臂复位
        ...
        if self.execution_count >= 4 or time.time() - self.start_time > 420: # 480秒等待时间
            ...
            time.sleep(1)
            m_speed.rotate_to_left(3.33,0.52)
            time.sleep(0.8)
            m_speed.moveforward(5,0.4)
            ...
            if (target_info is not None) and (self.forward_time < (15-self.execution_count*2)):
                ...
                if y is not None and (screen_midpoint-45) < y < (screen_midpoint+35):
                    ...
                    if self.execution_count %2 == 0:
                        ...
                        print("restart1")
                    if self.execution_count %2  !=0: 
                        self.execute_command_and_restart2(result_class,p1,p2)
                        p1 = 0
                        p2 = 0
                        print("restart2")
                    break
